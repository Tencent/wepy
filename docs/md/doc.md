...
在任何页面(page)或者组件(component)中都可以通过`wepy.$indtance`访问app实例（wepy.$indtance程序运行之后会一直存在）
...
*注意*: 父子组件单向绑定，传递的数据为对象，子组件改变该数据，父组件界面不刷新，但是父组件数据以改变，假设子组件调用this.$parent.$apply()就能帮父组件刷新视图（按照单向绑定规则，子组件的数据改变后，父组件内的数据应该不会变。）
      如果传递的数据未字符串，则完全依照单向绑定的规则，（猜测父子组件之间的对象使用了浅拷贝，公用的一个对象。）
