...
由于Page页面实际上继承自Component组件(因此本质上页面也属于组件，后文如果未加特别说明，提到组件时也包括了页面)，除扩展了页面所特有的`config`配置以及特有的页面生命周期函数之外，其它属性和方法与Component一致，因此这里以Page页面为例进行介绍。
...
`computed`计算属性，本质上是一个应该有返回值的函数(没有返回值也不会报错，但这样就失去了其意义)，可直接被当作绑定数据来使用。因此类似于`data`属性，脚本中可通过`this.计算属性名`来引用，模板中也可通过`{{ 计算属性名 }}`来插值。

只要计算属性所在的组件中的数据发生了改变(也就是有脏数据)，从而触发了脏数据检查流程(详见后文有关`脏数据检查`的介绍)的运行，计算属性就会被重新计算。

计算属性适用于其所在组件中每当发生脏数据检查时，需要进行某些额外处理的情形。

需要注意的是，只要是计算属性所在的组件中的数据发生了改变，从而引发了脏数据检查流程的运行，就会被重新计算，而跟其所引用的数据(比如`data`对象中的某个属性)本身是否发生改变无直接关系；换言之，计算属性中所引用的数据发生了改变，自然会导致其被重新计算，然而即便所引用的数据未发生改变，而是其所在的组件中的其他数据发生了改变，也会导致重新计算。
...
props传值在WePY中属于父子组件之间传值的一种机制，包括静态传值(只可以单向传值)与动态传值(既可以单向传值，也可以双向传值)。
...
与静态传值只能父组件向子组件单向传值不同，动态传值既可单向传值，也可双向传值，而且单向传值又包括了父向子单向传值和子向父单向传值。下面先介绍单向动态传值。

&emsp;&emsp;1. 父向子单向动态传值：即父组件可随时改变子组件中的值。父组件`data`对象中某个属性值的修改会改变子组件`props`对象中对应属性的值。要实现父向子单向动态传值，必须在子组件标签需要传值的属性后加上`.sync`后缀。注意，父组件与子组件中的这两个属性其名称可以不一致，两者通过在父组件`template`模板部分中所插入的子组件标签中的属性(该属性即子组件`props`对象中的同名属性)进行绑定。

&emsp;&emsp;2. 子向父单向动态传值：即子组件可随时改变父组件中的值。子组件`props`对象中某个属性值的修改会改变父组件`data`对象中对应属性的值。要实现子向父单向动态传值，必须将子组件`props`中的该需要动态传值的同名对象属性中的`twoWay`属性的值设为`true`。注意，子组件与父组件中的这两个属性其名称可以不一致，两者通过在父组件`template`模板部分中所插入的子组件标签中的属性(该属性即子组件`props`对象中的同名属性)进行绑定。

那如果子组件标签中的属性后既加了`.sync`后缀，同时子组件`props`中对应的同名对象属性的`twoWay`属性的值又为`true`，就实现了双向动态传值。
...
注意，这些父组件中的内容分发标签(比如view标签)必须具有`slot`属性，并且其值为子组件中对应的插槽名称，这样父组件内容分发标签中的内容会覆盖掉子组件对应插槽中的默认内容(如果有默认内容的话，没有默认内容则相当于插入了父组件中的内容，或者说父组件中的内容覆盖掉了子组件中原来的空字符串)，这就是所谓的内容分发，实质上就是父组件向子组件传递数据(即传值)的一种形式。
