...
const transformAssetMap = {
  'src': ['audio', 'image', 'video']
};
...
exports = module.exports = function () {
  ...
  Object.keys(transformAssetMap).forEach(attr => {

    this.register('template-parse-ast-attr-' + attr, function parseAssetUrl ({item, name, expr, ctx}) {
      
      let parsed = {};

      if (!transformAssetMap[attr].includes(item.name)) {
        // ignore asset transform
        // e.g: <web-view src="https://mp.weixin.qq.com/"></web-view>
        parsed.attr = { attrs: { [attr]: expr } };
        return parsed.attr;
      }
      ...
      parsed.attr = { attrs: { [attr]: parsed.url } };
      return parsed.attr;
    })
  })
}
