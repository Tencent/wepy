    ...
    build(props) {
      ...
      if (typeof(props) === 'string') {
        ...
        Object.keys(props).forEach(p => {
          ...
          if (rst[p].type && toString.call(rst[p].type) !==
            '[object Array]')
...
export default class {
      ...
      for (key in this.$props) {
        for (binded in this.$props[key]) {
          if (/\.sync$/.test(binded)) { // sync goes to mapping
            ...
            this.$mappingProps[this.$props[key][binded]][key] = binded.substring(
              7, binded.length - 5);
          }
        }
      }
      ...
      for (key in props) {
        if (keyCheck(this, key)) {
          ...
          if ($parent && $parent.$props && $parent.$props[this.$name]) {
            ...
            binded = $parent.$props[this.$name][`v-bind:${key}.once`] ||
              $parent.$props[this.$name][`v-bind:${key}.sync`];
            if (binded) {
              if (typeof(binded) === 'object') {
                ...
                if (binddata && (typeof binddata === 'object' || typeof binddata ===
                    'string')) {
  ...
  $broadcast(evtName, ...args) {
    let com = this;
    let $evt = typeof(evtName) === 'string' ? new event(evtName, this,
      'broadcast') : $evt;
    let queue = [com];
    ...
    if (this.$parent && this.$parent.$events && this.$parent.$events[this.$name]) {
      ...
      if (method && this.$parent.methods) {
        ...
        if (typeof(fn) === 'function') {
          ...
          throw new Error(
            `Invalid method from emit, component is ${this.$parent.$name}, method is ${method}. Make sure you defined it already.\n`
          );
        }
      }
    }
          ...
          if (!util.$isEqual(this[k], originData[k])) { // compare if new data is equal to original data
            ...
            if (this.watch) {
              if (this.watch[k]) {
                ...
                } else if (typeof this.watch[k] === 'string' && typeof this.methods[
                    k] === 'function') {
            ...
            if (this.$mappingProps[k]) {
              Object.keys(this.$mappingProps[k]).forEach((changed) => {
                ...
                if (typeof(mapping) === 'object') {
                  this.$parent.$apply();
                } else if (changed === 'parent' && !util.$isEqual(this.$parent
                    .$data[mapping], this[k])) {
                  this.$parent[mapping] = this[k];
                  this.$parent.data[mapping] = this[k];
                  this.$parent.$apply();
                } else if (changed !== 'parent' && !util.$isEqual(this.$com[
                    changed].$data[mapping], this[k])) {
...
function keyCheck(vm, k) {
  if (typeof vm[k] === 'function') {
    console.warn('You are not allowed to define a function "' + k +
      '" in data.');
    return 0;
...
function getEventsFn(comContext, evtName) {
  let fn = comContext.events ? comContext.events[evtName] : (comContext.$events[
    evtName] ? comContext.$events[evtName] : undefined);
