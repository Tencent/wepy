...
let PAGE_EVENT = ['onLoad', 'onReady', 'onShow', 'onHide', 'onUnload',
  ...
  allMethods.forEach((method, i) => {
    config[com.$prefix + method] = function(e, ...args) {
      ...
      if (e.currentTarget && e.currentTarget.dataset) {
        ...
        while (tmp['wpy' + method.toLowerCase() + (p = String.fromCharCode(
            65 + paramsLength++))] !== undefined) {
      ...
      com.$mixins.forEach((mix) => {
        mix.methods[method] && (mixRst = mix.methods[method].apply(
          com, args.concat(evt)));
      });
      com.$apply();
...
export default {
  $createApp(appClass, appConfig) {
    ...
    $createPage(pageClass, pagePath) {
      ...
      if ((typeof pagePath === 'boolean' && pagePath) || (arguments.length ===
          3 && arguments[2] === true))
        config.$page = page;
      ...
      if ([].concat(page.$mixins, page).findIndex(mix => mix['onPageScroll']) ===
        -1) {
